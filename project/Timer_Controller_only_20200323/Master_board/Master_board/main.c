...
/*---------------------------------------------------------------------*/
/*************	XXX_MC204S_K8_V10	**************
可控硅延时控制器:单路延时控制器-可扩展3路
单路延时控制器说明：
1、PT2272 遥控+按键控制。
2、可控硅控制强电无电火花，安全可靠。
3、模式：单分->双秒->双分
4、扩展的三路可以使用遥控器使能和关闭。掉电可保存。同时受延时控制器控制。

硬件资源：
1、TM1650 驱动0.36寸四位共阴极数码管，实现K1-K4 键盘。
2、595控制LED灯显示和扩展口光耦。
3、PT2272 遥控功能接口。
4、BT05蓝牙uart接口。
...
static u8 	Display_Code[2]={0xff,0x00};		//0：光耦、led状态输出 2:端口使能状态。
...
void ComX_Set(LogicType type,Hc595Bit Xbit)
{
	...
	if(type==ON)
		...
		Display_Code[0] = (~COM_MASK) | val;//COM位或1;
	else if(OFF_ALL == type)
		Display_Code[0] = COM_MASK & val;//COM位与0
	else if(OFF_ON == type)
...
{
	u8 val;
	val = Display_Code[1];//用于使能级联的1-6位
	if(type==ON)
...
void LedX_Set(LogicType type,Hc595Bit Xbit)
{
	...
	if(type==ON)
		...
		Display_Code[0] = (~LED_MASK) | val;//LED位或1
	else if(OFF_ALL == type)
		Display_Code[0] = LED_MASK & val;//LED位与0
	else if(OFF_ON == type)
		Display_Code[0] = reversebit(val,Xbit);//翻转
}
void Date_Transform(u8 num1,u8 num2)
{	
	...
	{
		LedX_Set(OFF_ALL,(Hc595Bit)7);//关闭所有LED;
		return ;
	}
...
{ 
	if(!E2promDis)
	{
		LedX_Set(OFF,RUN_MODE_LED);//正常运行时设置灯关闭;	
		return;
	}	
	if(LedFlash)
		LedX_Set(OFF_ON,RUN_MODE_LED);//设置灯闪烁;	
		//key_led_reverse();
...
void KeyProcess(KeyEnum key)
{
	...
	switch(key) //根据不同的按键触发序号执行对应的代码
	{
		case KeyOffTime:     //【关断时间设置】
			...
			//LedX_Set(OFF_ON,RUN_MODE_LED);//【设置灯】取反;	
...
void Channle_Sw(void)
{
	switch(SysRun.Mode)
	{
		case DUAL_MINU_MODE:
			...
			LedX_Set(ON,DUAL_MIN_LED);  //【LED5】双分灯亮;
			LedX_Set(OFF,DUAL_SEC_LED); //【LED5】取反;
			LedX_Set(OFF,ONLY_MIN_LED); //【LED5】取反;
			break;
		case ONLY_MINU_MODE:
			RunTime.tMode = KEY_TIME_60S;//单分,60s
			LedX_Set(ON,ONLY_MIN_LED);  //【LED5】单分灯亮;
			LedX_Set(OFF,DUAL_MIN_LED); //【LED5】取反;
			LedX_Set(OFF,DUAL_SEC_LED); //【LED5】取反;
			break;
		case DUAL_SEC_MODE:
			RunTime.tMode = KEY_TIME_1S;//双秒,1s
			LedX_Set(ON,DUAL_SEC_LED);  //【LED5】双秒灯亮;
			LedX_Set(OFF,DUAL_MIN_LED); //【LED5】取反;
			LedX_Set(OFF,ONLY_MIN_LED); //【LED5】取反;
			break;
		default:	
	...
	switch(SysRun.Step)
	{
		case TURN_ON_MODE://导通倒计时
			vGu8TimeFlag_1 = 1;
			ComX_Set(ON,OUT1); //【本路】开;
			Com_Match(ON,Display_Code[1]);//使能者运行
			if(vGu32TimeCnt_1>=RunTime.tMode)
			{
				...
				if(RunTime.on<=0){
					SysRun.Step++;
					vGu8TimeFlag_1 = 0;
					RunTime.off = E2PROM_Strings[OFF_TIME_CFG];//赋值EEPROM值
		...
		case TURN_OFF_MODE://关断倒计时
			vGu8TimeFlag_1 = 1;
			ComX_Set(OFF,OUT1);//【本路】关;
			Com_Match(OFF,Display_Code[1]);//使能者运行
			if(vGu32TimeCnt_1>=RunTime.tMode)
			{
				vGu32TimeCnt_1 = 0;
				RunTime.off--;
				if(RunTime.off <= 0){
					...
					vGu8TimeFlag_1 = 0;
					RunTime.on = E2PROM_Strings[ON_TIME_CFG];//赋值EEPROM值
				}
			}
			break;
